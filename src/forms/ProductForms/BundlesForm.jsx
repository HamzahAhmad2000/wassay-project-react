...
import { getBundleProducts, getRawMaterials } from "/src/APIs/ProductAPIs";
import { postBundle } from "/src/APIs/ProductAPIs";

const BundlesForm = ({ mode = "add" }) => {
  ...
  useEffect(() => {
    async function fetchProducts() {
      try {
        const response = await getBundleProducts();
        setBundleProducts(response || []);
  ...
  const validateForm = () => {
    ...
    if (formData.products_used.length === 0) {
      setError("Please add at least one bundle");
      return false;
    }
    return true;
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await postBundle(formData)
      if (!response.ok) {
        const errorData = await response.json();
        setError(`Failed to create bundle: ${errorData.error || "An error occurred"}`);
      ...
      if (data.id) {
        setSuccess(
          mode === "add" ? "Bundle created successfully!" : "Bundle updated successfully!"
        );
      }
      ...
      setTimeout(() => navigate("/bundles"), 1500);
  ...
  return (
    <div className="form-container max-w-4xl mx-auto bg-white p-6 rounded-lg shadow-md">
      <h2 className="form-heading text-2xl font-bold mb-6 text-blue-800">
        {mode === "add" ? "Add Bundles" : "Edit Bundles"}
      </h2>
      ...
      <form onSubmit={handleSubmit} className="product-form space-y-6">
        ...
        <div className="bg-gray-50 p-6 rounded-lg border border-gray-200">
          <h3 className="text-lg font-semibold mb-4 text-gray-800">1. Select Included Product</h3>
